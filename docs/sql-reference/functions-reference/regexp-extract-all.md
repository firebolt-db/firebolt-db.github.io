...
```sql
SELECT
	REGEXP_EXTRACT_ALL('Hello Year 2023 DEF', '[A-Za-z]+');
```
**Returns**: `["Hello","Year","DEF"]`
...
```sql
SELECT
	REGEXP_EXTRACT_ALL('Learning about #REGEX in #Firebolt 2023', '([A-Z])[a-z]+');
```
...
```sql
SELECT
	REGEXP_EXTRACT_ALL('Learning about #REGEX in #Firebolt 2023', '([A-Z])[a-z]+', '', 1);
```
**Returns**: `["L","F"]`
