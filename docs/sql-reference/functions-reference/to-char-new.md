...
| `MONTH`           | Full upper case month name (blank-padded to 9 chars)                                              | <code>TO_CHAR(CURRENT_TIMESTAMPTZ, 'MONTH'); --> 'MARCH&nbsp;&nbsp;&nbsp;&nbsp;'</code> | <!--- We explicitly need the <code> tags here to make sure we display the right amount of spaces (&nbsp;) in the example. Multiple spaces are generally removed in inline-code backticks. -->
| `Month`           | Full capitalized month name (blank-padded to 9 chars)                                             | <code>TO_CHAR(CURRENT_TIMESTAMPTZ, 'Month'); --> 'August&nbsp;&nbsp;&nbsp;'</code>      |
...
| `DAY`             | Full upper case day name (blank-padded to 9 chars)                                                | <code>TO_CHAR(CURRENT_TIMESTAMPTZ, 'DAY'); --> 'TUESDAY&nbsp;&nbsp;'                    |
| `Day`             | Full capitalized day name (blank-padded to 9 chars)                                               | <code>TO_CHAR(CURRENT_TIMESTAMPTZ, 'Day'); --> 'Wednesday'</code>                       |
| `day`             | Full lower case day name (blank-padded to 9 chars)                                                | <code>TO_CHAR(CURRENT_TIMESTAMPTZ, 'day'); --> 'thursday&nbsp;'</code>                  |
