...
| `<replacement>`    | The string to replace the matching pattern found in the input. This argument can include the following special sequences: <ul><li> `\&` - Specifies the input substring matching the entire pattern should be replaced.</li><li> `\n` - Where n is a digit from 1 to 9, specifies the input pattern matching the nth subexpression of the pattern should be replaced</li><li> `\\` - results in a single \</li><li> `\c` - Specifies for any other character, c results in the same sequence \c</li><\ul><p/>Note, that for string literals the above escaping rules apply *after* string literals escaping rules for `\`. See examples below. |
