...
* This optimization is applied automatically to any join-queries for which the right-hand side is directly a table (not a view or a subselect).
* Note that this does not apply to queries which contain restrictions to right-hand table fields.
* The first run of an eligible query still does the full join computation, subsequent runs of queries of the same pattern (e.g., may have different restrictions on the left-side) profit from the previously computed "join index" held in RAM.
